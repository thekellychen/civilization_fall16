    ...
    public BuildingMenu() {
        ...
        invest.setOnMouseClicked(e -> {
        	if (GameController.getCivilization().getTreasury().getCoins() >= 25) {
                ...
                newAlert.setHeaderText("Sorry, you cannot invest in this tile");
        ...
        demolish.setOnMouseClicked(e -> {
            if (tile.getOccupant() instanceof Settlement && GameController.getCivilization().getNumSettlements() <= 1) {
            	Alert newAlert = new Alert(Alert.AlertType.INFORMATION);
                newAlert.setHeaderText("Sorry, you cannot demolish your only settlement");
